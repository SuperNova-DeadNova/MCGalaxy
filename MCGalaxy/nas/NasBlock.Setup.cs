...
namespace NotAwesomeSurvival
{
    public partial class NasBlock
    {
        ...
        public static void Setup()
        {
            ...
            blocks[i].childIDs = new List<ushort>();
            ...
            blocks[i] = new NasBlock(i, Material.Liquid, int.MaxValue);
            blocks[i].collideAction = LavaCollideAction();
            blocks[i].disturbedAction = FloodAction(new ushort[] { 11 });
            i = 1; //Stone
            ...
            blocks[i].dropHandler = (NasPlayer, dropID) =>
            {
                return new Drop(197, r.Next(2, 5));
            };
            ...
            i = 652;
            blocks[i] = new NasBlock(i, Material.Organic, 3);
            blocks[i].interaction = EatInteraction(set, 0, -5f);
            i = 648; //Apple
            blocks[i] = new NasBlock(i, Material.Organic, 3);
            blocks[i].disturbDelayMin = fallSpeed;
            blocks[i].disturbDelayMax = fallSpeed;
            blocks[i].disturbedAction = FallingBlockAction(Block.FromRaw(i));
            blocks[i].interaction = EatInteraction(set, 0, 1f);
            i = 702; //Peach
            blocks[i] = new NasBlock(i, Material.Organic, 3);
            blocks[i].disturbDelayMin = fallSpeed;
            blocks[i].disturbDelayMax = fallSpeed;
            ...
            blocks[i] = new NasBlock(i, Material.Organic, 3);
            blocks[i].interaction = EatInteraction(set4, 0, -2.5f);
            for (i = 484; i < 524; i++)
            {
                blocks[i] = new NasBlock(i, Material.Stone, DefaultDurabilities[(int)Material.Stone], 1);
            }
        ...
        public static Func<NasPlayer, ushort, Drop> CustomDrop(ushort clientushort, int amount)
        {
            ...
            {
                return new Drop(clientushort, amount);
            };
        }
